...
type Order = { id: number; status: string; totalPrice: number; createdAt: string; deliveryAddress?: string | null; product: { name: string }; user: { fullName: string; email: string } }
...
function OrdersTab() {
  ...
  const [error, setError] = useState('')
  const loadOrders = useCallback(() => { let alive = true; setOrders(null); setError(''); api.get<{ orders: Order[] }>('/orders/all').then((r) => { if (alive) setOrders(r.data.orders) }).catch((e) => { if (alive) { setOrders([]); setError(apiErrorMessage(e, 'Could not load orders from the backend.')) } }); return () => { alive = false } }, [])
...
function AdminConversationPanel({ conversation, conversations, onSelect }: { conversation: SupportConversation | null; conversations: SupportConversation[]; onSelect: (conversation: SupportConversation | null) => void }) {
  const [messages, setMessages] = useState<Array<{ id: string; body: string; senderId: string }>>([])
  const [draft, setDraft] = useState('')
  const [sending, setSending] = useState(false)
  React.useEffect(() => { if (!conversation) return; api.get<{ messages: Array<{ id: string; body: string; senderId: string }> }>(`/support/conversations/${conversation.id}/messages`).then((response) => setMessages(response.data.messages)).catch(() => setMessages([])) }, [conversation])
